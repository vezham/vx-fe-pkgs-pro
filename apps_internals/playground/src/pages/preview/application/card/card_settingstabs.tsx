...
import {
  ...
  SwitchCellProps,
  switchtoggleProps
} from '@vx-pro/application-card-settingstabs'
import React from 'react'

const card_settingstabs = () => {
  ...
  const navtoggle: switchtoggleProps[] = [
    {
  ...
  const notificationdata: cardNotificationProps = {
  ...
  const securitydata: cardSecurityProps = {
  ...
  const accountdata: cardAccountProps = {
  ...
  const AccountDetails = ({ data }: { data: cardAccountProps }) => {
  ...
  const NotificationsSettings = ({ data }: { data: cardNotificationProps }) => {
    return (
  ...
  const SecuritySettings = ({ data }: { data: cardSecurityProps }) => {
    return (
      <Card className="w-full max-w-lg p-2">
        <CardHeader className="flex flex-col items-start px-4 pt-4 pb-0">
          <p className="text-large">{data.title}s</p>
          <p className="text-small text-default-500">{data.subtitle}</p>
        </CardHeader>
  ...
  const data: cardProps[] = [
    {
      ...
      children: <AccountDetails data={accountdata} />
    },
    {
      ...
      children: <NotificationsSettings data={notificationdata} />
    },
    {
      ...
      children: <SecuritySettings data={securitydata} />
    }
  ]

  return (
    <div>
      <CardSettingsTabs data={data} />
    </div>
  )
}

export default card_settingstabs
